...
% initiation). Currently only the tube with the highest PoI is set to be examined.

load('TubeOrderByPoI.mat'); 
Tubes = TubeOrderByPoI(1); % Tubes = TubeOrderByPoI(1:37)';


PostAsmtPlots = 0; % Producing plots to show the results from the analysis
...
TPL_Prob_Asmt_V7(Nb,Tubes,AnalysisType,Conduct_SA,SAParam,LHS_Bin,PostAsmtPlots);


end


function TPL_Prob_Asmt_V7(Nb,Tubes,AnalysisType,Conduct_SA,SAParam,LHS_Bin,CondPostAsmtPlots)
...
% Ideal number of parallel workers for 100,000 is 3, and as such
% there's one tube leftover. Hence the following addjustment

Group1 = 1:(N_Workers.*floor(length(Tubes)/N_Workers));
Group2 = Group1(end)+1:length(Tubes);

GrouptedTubes1 = reshape(Tubes(Group1),[N_Workers,length(Tubes(Group1))/N_Workers]);
GrouptedTubes2 = [Tubes(Group2)' ;zeros(N_Workers-length(Group2),1)];

GrouptedTubes = [GrouptedTubes1, GrouptedTubes2];


N_Groups = size(GrouptedTubes,2);

N_Groups= 1;

for G = 1:N_Groups
    
    if sum(GrouptedTubes(:,G)==0) == 0
        SubGroup = GrouptedTubes(:,G);
    else
        SubGroup = GrouptedTubes(GrouptedTubes(:,G)~=0,G); 
    end
    
    for T = 1:length(SubGroup)
        ...
        switch AnalysisType
            case 'Probabilistic'
                [TotalDamage{T},...
                    ...
                    SA_Indicies{T},...
                    CovMat{T},...
                    SO_Stress_SA{T}] = R5_Assessment(GrouptedTubes(T,G),WorkSpace_Filename,LHS);
                
            case 'Deterministic'
                TotalDamage{T} = R5_Assessment(Tubes(T),WorkSpace_Filename,LHS);
        end
        
    end
    
    SubResultsfilename = ['Res_' AnalysisType '_Nb_' num2str(Nb) '_TubeGroup_' num2str(G) '_of_' num2str(N_Groups) '.mat'];
    
    if Nb <= 10000
        save(SubResultsfilename)
    elseif Nb > 10000
        save(SubResultsfilename,'Tubes','TotalDamage','Mean_Log_Damage','STD_Log_Damage','Prob_Fail') % Saving a smaller data set when dealing with large trials
    end
    
     clear TotalDamage Mean_Log_Damage STD_Log_Damage Prob_Fail SA_Indicies CovMat SO_Stress_SA
...
%% Collate the results into one file

cd('C:\Users\nz9512\Google Drive\1. PhD Work\3. TPL FEA\PlayGround\11. Probabilistic Assessment\Final code\Final Results')
...
for G = 1:N_Groups
    ...
    load(['Res_' AnalysisType '_Nb_' num2str(Nb) '_TubeGroup_' num2str(G) '_of_' num2str(N_Groups) '.mat']);
    
    for NSubG = 1:length(TotalDamage)
        All_Prob_Fail{N_Workers*(G-1) + NSubG} = Prob_Fail{NSubG};
        All_TotalDamage{N_Workers*(G-1) + NSubG} = TotalDamage{NSubG};
        Median_TotalDamage(N_Workers*(G-1) + NSubG) = median(TotalDamage{NSubG});
        
    end
    
    Tubes(Tubes==0) = [];
    
    save(['Res_' AnalysisType '_Nb_' num2str(Nb) '_All_Tubes_SMRG_Server.mat'],'Tubes','All_Prob_Fail','All_TotalDamage','Median_TotalDamage')
    
end

load(Resultsfilename)

TotalDamage = All_TotalDamage;
Prob_Fail = All_Prob_Fail;

%% Post analysis: more plots and SA

if CondPostAsmtPlots
    switch AnalysisType
        case 'Probabilistic'
            for T = 1:length(Tubes)
                
%                 disp(['Prob Median Total Damage on Tube ' num2str(Tubes(T)) ' = ' num2str(median(TotalDamage{T}))])
%                 
%                 
%                 % Plotting the PoF of each tube against cycle number
%                 figure;whitebg('w'); set(gcf,'color','w');
%                 
%                 plot(Prob_Fail{T}); hold on; xlabel('Number of Cycles'); ylabel('PoF of Individual Tubes')
                
                

                % Calculating the PoF for the whole TPL based on a 'first-pass-the-post' approach
                
                %                             TotalDamage_TPL = [];
                %
                %                             for T = 1:length(Tubes)
                %                                 TotalDamage_TPL = horzcat(TotalDamage_TPL, TotalDamage{T});
                %                             end
                %
                %                             PoF_TPL = sum((sum(TotalDamage_TPL>=1,2) >=1))./Nb;
                %
                %                             disp(['PoF for Tube ' num2str(Tubes(T)) ' = ' num2str(PoF_TPL)])
                %
                %                             % Plotting the binomial distribution based on the total number of tubeplates and the calcuated PoF
                %
                %                             N_tot = 2*2*2*4*4*1; % 2 Stations, 2 reactors each, 2 'something', 4 boiler pods each, 4 headers each, 1 tubeplate each
                %
                %                             PlotBinomialDistribution(PoF_TPL,N_tot)
                
                
%                 % Looking at the covariance matrix
%                 format short
%                 
%                 MeanCovMat = zeros(size(CovMat{T}{1}));
%                 
%                 for i = 1:length(CovMat{T})
%                     MeanCovMat = MeanCovMat + CovMat{T}{i};
%                 end
%                 
%                 MeanCovMat = MeanCovMat./length(CovMat{T});
%                 
%                 Parameters_Discription = {'Total damage','Ductility', 'Creep-rate','Zeta_p' ,'A (RO)' ,'S_y','E', '\alpha','N_f','Sigma_B','SU Str','RT Str','SU Temp','RT Temp','SO Temp','SO Str'};
%                 
%                 CorrLimit = 0.1;
%                 
%                 [I,J] = find((abs(MeanCovMat)>=CorrLimit) .* (MeanCovMat ~= 1 ));
%                 
%                 [IC] = find((abs(MeanCovMat)>=CorrLimit) .* (MeanCovMat ~= 1 ));
%                 
%                 
%                 J(I==1) = [];
%                 IC(I==1) = [];
%                 I(I==1) =[];
%                 
%                 [CorrSA,ICC] = sort(abs(MeanCovMat(IC)));
%                 
%                 [~,UI] = unique(CorrSA);
%                 
%                 CovMat_Results.ParamDiscrip = [Parameters_Discription(I(ICC(UI)))' Parameters_Discription(J(ICC(UI)))'];
%                 
%                 CovMat_Results.InterParamCorrs = CorrSA(UI);
                
                
                % Examaing parameter sensitivity through correlations
                
   %             Percentage_Contribution_Corr = CorrSensitivitityAnalysis(Tubes(T),Tubes,SA_Indicies,CovMat,SO_Stress_SA,TotalDamage,LHS);
...
%                 set(gcf, 'Position', [50 50 750 750]);
                ...
                % Fitting a distribution and estimating the PoF by integration
                
                MLE_Resutls = UoB_DistFitter_MLE(TotalDamage{T},'Lognormal');
                LRM_Resutls = UoB_DistFitter_LRM(TotalDamage{T},'Lognormal');
                
                Estimated_PoF(T,:) = [ sum(TotalDamage{T}>=1)./length(TotalDamage{T}) 1 - cdf(MLE_Resutls.pd,1)  1 - cdf(LRM_Resutls.pd,1)];
        ...
        case 'Deterministic'
            ...
            if length(Tubes) == 1
                disp(['Det Total Damage on Tube ' num2str(Tubes(T)) ' = ' num2str(TotalDamage{T})])
            else
                figure; plot(cell2mat(TotalDamage),'xk')
            end
    end 
end
...
function [TotalDamage,Mean_Log_Damage,STD_Log_Damage,Prob_Fail,SA_Indicies,CovMat,SO_Stress_SA] = R5_Assessment(Assess_Tube,WorkSpace_Filename,LHS)
...
[SU_Strs_Samples,SU_Temps_Samples,RT_Strs_Samples,RT_Temps_Samples] = AssignCycleTransients(Nb,LHS,CycleNo,Tube,Plot_Transient_Histograms,AnalysisType,Conduct_SA,SAParam);
...
for Event_No = 1:length(Dwell_Times)
    ...
    if Dwell_Times(Event_No) ~= 0
        ...
        if CycleEventNo(Event_No) == 1
            ...
            switch AnalysisType
                case 'Probabilistic'
                    % This is for the case a SA is being conducted
                    if Conduct_SA
                        switch SAParam
                            case 14
                                Sorted_Event_Temp = sort(Event_Temp);
                                Event_Temp = Sorted_Event_Temp(LHS(:,14));
                        end
                    end
                case 'Deterministic'
                    Sorted_Event_Temp = sort(Event_Temp);
        ...
        TotalDamage_By_Cycle = Fatigue_Damage + Creep_Damage; % This is accumulated damage
        
        Increment_of_TotalDamge_By_Cycle = TotalDamage_By_Cycle - Initial_TotalDamage_By_Cycle;
        
        Initial_TotalDamage_By_Cycle = TotalDamage_By_Cycle; % Initial as in at the start of the next cycle
        ...
        Prob_Fail(Cycle_Counter) = sum(TotalDamage_By_Cycle>1)./Nb;
        
        Mean_Log_Damage(Cycle_Counter) = mean(log10(TotalDamage_By_Cycle));
        STD_Log_Damage(Cycle_Counter) = std(log10(TotalDamage_By_Cycle));
...
function Conduct_Sigma_SA(Tube)
...
load('SA_Sigma_Based_Results_V2.mat')
cd(Direct)

for SAParam = 8:15
    
    parfor NBin = 1:N_SA_Samples
        LHS_Bin(SAParam,NBin) = round(NBin/N_SA_Samples*Nb);
        [Sigma_SA_TotalDamage{SAParam,NBin}] =  TPL_Prob_Asmt_V7(Nb,Tube,AnalysisType,Conduct_SA,SAParam,LHS_Bin(SAParam,NBin),PostAsmtPlots);
    end
    
    Direct = cd;
    cd('C:\Users\nz9512\Google Drive\1. PhD Work\3. TPL FEA\PlayGround\11. Probabilistic Assessment\SA')
    save('SA_Sigma_Based_Results_V2.mat','Sigma_SA_TotalDamage')
    cd(Direct)
...
for SAParam = 1:15
    Var_Total_Damage{SAParam} = 0;
    STD_Total_Damage{SAParam} = 0;
end


for SAParam = 1:15
    TotalDamage{SAParam} = TPL_Prob_Asmt_V7(Nb,Tube,AnalysisType,Conduct_SA,SAParam,LHS_Bin,PostAsmtPlots);
    
    Var_Total_Damage{SAParam} = var(cell2mat(TotalDamage{SAParam}));
    STD_Total_Damage{SAParam} = std(cell2mat(TotalDamage{SAParam}));
end


% Conduct the base line assessment

SAParam = 0;
Conduct_SA = 0;
TotalDamage = TPL_Prob_Asmt_V7(Nb,Tube,AnalysisType,Conduct_SA,SAParam,LHS_Bin,PostAsmtPlots);
Var_Total_Damage_All_Params = var(cell2mat(TotalDamage));
STD_Total_Damage_All_Params = std(cell2mat(TotalDamage));

Direct = cd;
cd('C:\Users\nz9512\Google Drive\1. PhD Work\3. TPL FEA\PlayGround\11. Probabilistic Assessment\SA')
save('SA_Var_Based_Results.mat','STD_Total_Damage','Var_Total_Damage','Var_Total_Damage_All_Params','STD_Total_Damage_All_Params')
cd(Direct)
...
function [Prob_SU_Strs,Prob_SU_Temps,Prob_RT_Strs,Prob_RT_Temps] = AssignCycleTransients(Nb,LHS,CycleNo,Tubes,Plot_Transient_Histograms,AnalysisType,Conduct_SA,SAParam)
...
[Prob_SU_Strs,~,~,~] = AssignCycleTransients(Nb,LHS,CycleNo,Tube,0,AnalysisType,Conduct_SA,SAParam);
...
%% Script for calculating the Spindler Fraction
...
function [Varep_f_Mean,Varep_f_SMDE_LB] = Removing_NonRealistic_Ductilities(Varep_f_Mean,Varep_f_SMDE_LB)

if sum(Varep_f_Mean > Varep_f_SMDE_LB) ~= 0
    Valid_Ductilities = [Varep_f_Mean(Varep_f_Mean > Varep_f_SMDE_LB) Varep_f_SMDE_LB(Varep_f_Mean > Varep_f_SMDE_LB)];
    No_Missing = length(Varep_f_Mean) - size(Valid_Ductilities,1);
    Valid_Ductilities = [ones(No_Missing,1).*Valid_Ductilities(1,1) ones(No_Missing,1).* Valid_Ductilities(1,2);Valid_Ductilities];
else
    Valid_Ductilities = [Varep_f_Mean  Varep_f_Mean] ;
end

Varep_f_Mean = Valid_Ductilities(:,1);
Varep_f_SMDE_LB = Valid_Ductilities(:,2);
